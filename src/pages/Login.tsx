...
const Login = () => {
	...
	const registroForm = useForm<UsuarioTipo>( {resolver: zodResolver(registroSchema)})
	const loginForm = useForm<UsuarioTipo>( {resolver: zodResolver(loginSchema)})

	const { register: registerLogin, handleSubmit: handleSubmitLogin, formState:{errors: errorsLogin} } = loginForm
	const { register: registerRegistro, handleSubmit: handleSubmitRegistro, formState:{errors: errorsRegistro} } = registroForm
		...
		<div className={estilos.container}>
			<div className={estilos.blur}>
				...
				<div className={`${estilos.modal} ${ativo ? estilos.ativo : ""}` }>
					...
					{/*---------------- LOGIN ----------------*/}
					<form className={estilos.formLogin} onSubmit={handleSubmitLogin(autenticarUsuario)}>
						<div className={estilos.login}>
							...
							<div className={estilos.campo}>
								<input placeholder="Email" type='text' {...registerLogin('email')} />
								{ errorsLogin.email && <p className={estilos.mensagem}>{errorsLogin.email.message}</p> }
							</div>

							<div className={estilos.campo}>
								<input placeholder='Senha' type='password' {...registerLogin('senha')}/>
								{ errorsLogin.senha && <p className={estilos.mensagem}>{errorsLogin.senha.message}</p> }
							</div>
					...
					{/*---------------- REGISTRO ----------------*/}
					<form className={estilos.formRegistrar} onSubmit={handleSubmitRegistro(criarUsuario)}>
						<div className={estilos.registrar}>
							...
							<div className={estilos.campo}>
								<input placeholder='Nome' type='text' {...registerRegistro('nome')} />
								{ errorsRegistro.nome && <p className={estilos.mensagem}>{errorsRegistro.nome.message}</p> }
							</div>

							<div className={estilos.campo}>
								<input placeholder="Email" type='text' {...registerRegistro('email')} />
								{ errorsRegistro.email && <p className={estilos.mensagem}>{errorsRegistro.email.message}</p> }
							</div>

							<div className={estilos.campo}>
								<input placeholder='Senha' type='password' {...registerRegistro('senha')} />
								{ errorsRegistro.senha && <p className={estilos.mensagem}>{errorsRegistro.senha.message}</p> }
							</div>
